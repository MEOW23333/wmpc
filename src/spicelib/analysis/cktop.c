...
static int
segment_warmup_load_rhsold(CKTcircuit *ckt, double gmin_target)
{
    int i, vector_size, tail_char;
    int valid = 0;
    char path[1024];
    const char *input_dir = getenv("SEGMENT_WARMUP_INPUT_DIR");
    const char *ckt_id = getenv("CKT_ID");
    FILE *fp = NULL;
    double *candidate = NULL;

    if (!input_dir || !ckt_id || gmin_target <= 0.0)
        return 0;

    snprintf(
        path,
        sizeof(path),
        "%s/segment_warmup_circuit_%s_time_%.17e_gmin_%.17e_rhsold.txt",
        input_dir,
        ckt_id,
        ckt->CKTtime,
        gmin_target
    ...
    vector_size = SMPmatSize(ckt->CKTmatrix);
    if (vector_size <= 0)
        goto cleanup;
    candidate = malloc(((size_t) vector_size + 1) * sizeof(*candidate));
    if (!candidate) {
        fprintf(stderr,
                "PALS_SEGMENT_WARMUP_INPUT_INVALID reason=allocation_failed expected_length=%d path=%s\n",
                vector_size, path);
        goto cleanup;
    }
    for (i = 1; i <= vector_size; i++) {
        if (fscanf(fp, "%le", &candidate[i]) != 1) {
            fprintf(stderr,
                    "PALS_SEGMENT_WARMUP_INPUT_INVALID reason=too_short_or_non_numeric expected_length=%d values_read=%d path=%s\n",
                    vector_size, i - 1, path);
            goto cleanup;
        }
    }
    while ((tail_char = fgetc(fp)) != EOF) {
        if (!isspace((unsigned char) tail_char)) {
            fprintf(stderr,
                    "PALS_SEGMENT_WARMUP_INPUT_INVALID reason=too_long_or_trailing_data expected_length=%d path=%s\n",
                    vector_size, path);
            goto cleanup;
        }
    }
    if (ferror(fp)) {
        fprintf(stderr,
                "PALS_SEGMENT_WARMUP_INPUT_INVALID reason=read_error expected_length=%d path=%s\n",
                vector_size, path);
        goto cleanup;
    }
    for (i = 1; i <= vector_size; i++)
        ckt->CKTrhsOld[i] = candidate[i];
    valid = 1;
cleanup:
    free(candidate);
    fclose(fp);
...
{
    const char *capture_path = getenv("SEGMENT_WARMUP_CAPTURE_PATH");
    const char *ckt_id = getenv("CKT_ID");
    FILE *fp = NULL;

    if (!capture_path || !ckt_id || gmin_val <= 0.0)
        return;

    fp = fopen(capture_path, "a");
    if (!fp)
        return;
              ...
              long int continuemode, int iterlim)
{
    ...
    stage_injected = segment_warmup_load_rhsold(ckt, ckt->CKTdiagGmin);

    for (;;) {
        double stage_gmin = ckt->CKTdiagGmin;
        if (ft_ngdebug)
        ...
        segment_warmup_log_stage(ckt, "dynamic_gmin", stage_gmin, iters, stage_injected, converged == 0);
...
spice3_gmin (CKTcircuit *ckt, long int firstmode,
             long int continuemode, int iterlim)
{
    int converged, i, iters, stage_injected;
    ...
    for (i = 0; i <= ckt->CKTnumGminSteps; i++) {
        ...
        converged = NIiter (ckt, ckt->CKTdcTrcvMaxIter);
        iters = ckt->CKTstat->STATnumIter - iters;
        segment_warmup_log_stage(ckt, "spice3_gmin", stage_gmin, iters, stage_injected, converged == 0);
    ...
    long int continuemode, int iterlim)
{
    ...
    stage_injected = segment_warmup_load_rhsold(ckt, ckt->CKTgmin);

    for (;;) {
        double stage_gmin = ckt->CKTgmin;
        if (ft_ngdebug)
        ...
        converged = NIiter(ckt, ckt->CKTdcTrcvMaxIter);
        iters = ckt->CKTstat->STATnumIter - iters;
        segment_warmup_log_stage(ckt, "new_gmin", stage_gmin, iters, stage_injected, converged == 0);
